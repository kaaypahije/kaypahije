...
import { Phone, Bot, X, Send, Headset } from "lucide-react";
    ...
    <>
      <div className="fixed bottom-4 right-3 md:bottom-6 md:right-6 z-[60] flex flex-col items-end gap-3">
        {menuOpen && (
          <>
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-full bg-primary text-primary-foreground shadow-glow hover:scale-110 transition-transform"
              style={{ height: 48, width: 48 }}
              aria-label="Chat"
            >
        ...
        <button
          ...
          className="inline-flex h-11 items-center gap-1.5 rounded-full bg-primary px-3 text-primary-foreground shadow-glow hover:scale-105 transition-transform"
          aria-label="Help"
        >
          {menuOpen ? <X className="h-4 w-4" /> : <Headset className="h-4 w-4" />}
          <span className="text-xs font-semibold">{menuOpen ? "Close" : "Help"}</span>
        </button>
      </div>
